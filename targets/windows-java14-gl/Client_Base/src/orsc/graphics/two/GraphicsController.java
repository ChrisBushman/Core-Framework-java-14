...
public class GraphicsController {
	...
	private void a(int var1, int[] var2, int var3, int var4, int var5, int var6, byte var7, int var8, int[] var9,
		...
		try {
			...
			// Skips both this engine's two established "no pixel here"
			// color-keys (see GLSceneRenderer.decodeTexel()): plain black,
			// checked here from the start, and magenta (0xF800FF) - added
			// after a settings-panel icon (whose own art uses magenta for
			// its transparent background, not black) came through as an
			// opaque solid magenta box instead of properly transparent,
			// since this loop only ever recognized black.
			for (int var13 = -var4; var13 < 0; var13 += var3) {
				int var14;
				for (var14 = var12; var14 < 0; ++var14) {
					var5 = var9[var6++];
					if (var5 == 0 || var5 == 0xF800FF) {
						++var8;
					} else {
						var2[var8++] = var5;
					}
			...
			try {
				...
				for (int i = -destHeight; i < 0; i += heightStep) {
					...
					for (int j = -destWidth; j < 0; ++j) {
						...
						srcStartX += scaleX;
						// See the other plot helper (a(...) above) for why
						// magenta (0xF800FF) is skipped alongside black.
						if (color != 0 && color != 0xF800FF) {
							dest[destHead++] = color;
						} else {
							++destHead;
						}
