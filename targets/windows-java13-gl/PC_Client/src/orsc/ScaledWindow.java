...
// setMinimumSize N/A in Java 1.4
	 ...
	 * assigned them (confirmed: 56x17, before any real content/scalar is
	 * set). resizeApplet()/validateAppletSize() must not be allowed to
	 * propagate that stale size into mudclient's real game dimensions -
	 * root-caused after it corrupted GraphicsController's pixelData buffer
	 * (via mudclient.reposition() -> getSurface().resize()) partway through
	 * a real login, crashing generateLandscapeModel()'s minimap copy with an
